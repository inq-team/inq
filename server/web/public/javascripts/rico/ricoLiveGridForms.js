...
Rico.TableEdit.prototype = {
  ...
  appendFormField: function(column, table) {
    ...
    Rico.writeDebugMsg('appendFormField: '+column.displayName+' - '+column.format.EntryType);
    ...
    hdr.id='lbl_'+column.format.FieldName;
    ...
    if (field) {
      if (column.format.onchange) field.onchange=RicoValidate.onchange.bindAsEventListener(RicoValidate,column.format.onchange);
  ...
  TESubmit: function(e) {
    ...
    for (i = 0; i < this.grid.columns.length; i++) {
      ...
      if (spec.onchange) {
        var validator=RicoValidate[spec.onchange];
        if (validator) {
...
/**
 * @singleton
 * Utility functions to validate form fields
 */
var RicoValidate = {
  
  onchange: function(e, ValidatorName) {
    var elem=Event.element(e);
    var validator=RicoValidate[ValidatorName];
    if (validator) validator(elem);
    return true;
  },
  
  DisplayErrMsg: function(elem, errmsg) {
    var msg=RicoTranslate.getPhrase(errmsg);
    var LblObj=$("lbl_"+elem.id);
    if (LblObj) msg+=" "+RicoTranslate.getPhrase("for")+" \"" + LblObj.innerHTML + "\"";
    alert(msg);
    setTimeout(function() { try { elem.focus(); elem.select(); } catch(e) {}; }, 10);
  },

  /**
   * Return true if TxtObj is empty or contains a valid integer
   */
  CheckInt: function(TxtObj) {
    var val=TxtObj.value;
    if (val=='' || val==parseInt(val)) return true;
    RicoValidate.DisplayErrMsg(TxtObj, "Please enter\t an integer value");
    return false;
  },

  /**
   * Return true if TxtObj is empty or contains a valid integer >= 0
   */
  CheckPosInt: function(TxtObj) {
    var val=TxtObj.value;
    if (val=='') return true;
    if (val==parseInt(val) && val>=0) return true;
    RicoValidate.DisplayErrMsg(TxtObj, "Please enter\t a positive integer value");
    return false;
  }
}
