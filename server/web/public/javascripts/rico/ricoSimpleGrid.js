...
showSimpleMenu: function(e,hideFunc) {
  ...
  var newLeft=Math.floor(td.offsetLeft-this.grid.scrollDiv.scrollLeft+td.offsetWidth/2);
  if (this.direction == 'rtl') {
    if (newLeft > this.width) newLeft-=this.width;
  } else {
    if (newLeft+this.width+this.options.margin > this.grid.scrollDiv.clientWidth) newLeft-=this.width;
  }
  this.div.style.visibility="hidden";
  this.div.style.display="block";
  var contentHt=this.div.offsetHeight;
  var newTop=Math.floor(elem.offsetTop-this.grid.scrollDiv.scrollTop+elem.offsetHeight/2);
  if (newTop+contentHt+this.options.margin > this.grid.scrollDiv.clientHeight)
    newTop=Math.max(newTop-contentHt,0);
  this.openPopup(this.grid.frzWi+newLeft,this.grid.hdrHt+newTop);
